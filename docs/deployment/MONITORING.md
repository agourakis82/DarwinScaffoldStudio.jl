...
# Verify
kubectl get pods -n darwin-scaffold-studio -l app=prometheus
kubectl get pods -n darwin-scaffold-studio -l app=grafana

# Access Grafana
kubectl port-forward svc/grafana 3000:3000 -n darwin-scaffold-studio
...
**Exposed at:** `http://darwin-scaffold-studio:9090/metrics`
...
```prometheus
# Pod metrics
kube_pod_status_phase{namespace="darwin-scaffold-studio"}
kube_pod_container_status_restarts_total{namespace="darwin-scaffold-studio"}

# Resource usage
container_cpu_usage_seconds_total{namespace="darwin-scaffold-studio"}
container_memory_usage_bytes{namespace="darwin-scaffold-studio"}
```
    ...
    groups:
      - name: darwin-scaffold-studio
        ...
        rules:
          ...
          # Pod not ready
          - alert: PodNotReady
            expr: kube_pod_status_phase{namespace="darwin-scaffold-studio", phase!="Running"} == 1
            for: 5m
            labels:
              severity: warning
            annotations:
          ...
          - alert: HighMemoryUsage
            expr: container_memory_usage_bytes{namespace="darwin-scaffold-studio"} / container_spec_memory_limit_bytes > 0.9
            for: 5m
            labels:
              severity: warning
            annotations:
...
# Last 100 lines
kubectl logs --tail=100 deployment/darwin-scaffold-studio -n darwin-scaffold-studio

# Logs from all pods
kubectl logs -l app=darwin-scaffold-studio -n darwin-scaffold-studio --all-containers=true

# Previous pod logs (if crashed)
kubectl logs deployment/darwin-scaffold-studio -n darwin-scaffold-studio --previous
```

**Grafana Loki (if installed):**
```
# Query syntax
{namespace="darwin-scaffold-studio", app="darwin-scaffold-studio"} |= "ERROR"
{namespace="darwin-scaffold-studio"} |= "GraphRAG"
{namespace="darwin-scaffold-studio"} | json | level="ERROR"
```
...
# Search for: darwin-scaffold-studio service
```
...
kubectl get pod -n darwin-scaffold-studio -o yaml | grep prometheus
...
kubectl port-forward svc/prometheus 9090:9090 -n darwin-scaffold-studio
...
kubectl edit deployment/prometheus -n darwin-scaffold-studio
...
**Issues:**
- GitHub: https://github.com/agourakis82/darwin-scaffold-studio/issues
